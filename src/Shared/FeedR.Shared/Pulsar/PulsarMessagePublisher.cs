...
{
    //TODO: extract Pulsar into app settings and dedicated options
    ...
    public PulsarMessagePublisher(ISerializer serializer, IHttpContextAccessor contextAccessor, ILogger<PulsarMessagePublisher> logger)
    {
        ...
        // the name of process executable, for example FeedR.Aggregator
        _producerName =
            Assembly.GetEntryAssembly()?.FullName?.Split(",")[0]?.ToLowerInvariant() ?? string.Empty;
    }

    public async Task PublishAsync<TMessage>(string topic, TMessage message) where TMessage : class, IMessage
    {
        ...
        _logger.LogInformation("Sent a message with Id: '{id}'.", messageId);
    }
}